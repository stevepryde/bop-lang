...
description = "Bop ships a very small set of built-in functions that are always in scope. They can't be shadowed by user-defined fns. Host-backed builtins (I/O, time) are provided separately by the embedding host — see `bop-sys`'s `StandardHost` for the reference implementation."
weight = 18
