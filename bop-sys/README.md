...
The standard host for the [Bop](https://github.com/stevepryde/bop-lang) programming language — a `BopHost` implementation that wires Bop up to the normal OS-backed conveniences (filesystem, stdio, environment, time) plus the bundled stdlib from [`bop-std`](https://crates.io/crates/bop-std).
...
- `import std.math` / `std.json` / `std.collections` / … → resolved via `bop-std`
...
- **WASM / no_std builds** — `bop-sys` depends on `std` for filesystem and time. Use `bop-lang` (and optionally `bop-vm` + `bop-std`) directly.
...
- [`bop-std`](https://crates.io/crates/bop-std) — the Bop stdlib modules this crate routes `import std.*` to
